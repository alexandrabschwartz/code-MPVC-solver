function [x_opt, f_opt, information] = solveMPVC_relaxation_posLB(problem, options)
...
%                   H(x) >= t^2,  phi(G(x),H(x);t) <= 0
% and solving the resulting NLP for decreasing values of t > 0.

% Enforcing such a positive lower bound on H(x) is popular in truss design 
% to ensure regularity of the stiffness matrix. In can only be used in
% conjunction with a relaxation of the condition G(x).*H(x), because
% otherwise the vanishing constraint cannot vanish.
...
% vanishing constraints, specify the relaxation function or the NLP solver, 
...
if options.slacks ~= true
    % define nonlinear reformulation without slack variables
    NLPproblem.cl = [problem.cl; t^2*ones(n_van,1); -inf(n_van,1)];   
else
    % define nonlinear reformulation with slack variables y = G(x), z = H(x)
    NLPproblem.xl = [problem.xl; -inf(n_van,1); t^2*ones(n_van,1)];
end
...
while (iterations == 0) || ((iterations < iterations_max) && (t > t_min) ...
    ...
    % update the initial point and lower bounds on H/z
    if options.slacks ~= true
